...
| `liveStage` | The TikTok-style stage: `StageGrid` splits the screen per publisher (1 → full-bleed host, 2 → side-by-side, 3-4 → 2×2, 5+ → 3-wide; the room's ONE main `<video>` rides in as `hostSlot` so attach effects survive layout changes). Camera-off cells wear a profile PLATE — own cells from local state, remote ones via `useVideoPresence` (canvas luma sampling; a disabled WebRTC track still sends black frames, so "camera off" is invisible on the wire). `useStageVideos` keeps one WHEP per remote guest, keyed by userId+whepUrl, self-redialling on failure. Plus floating reactions, gift picker/splash/supporters, request queue, invite banner. The host is NEVER a WHEP cell — a second subscription to their path would double their audio. | [liveStage.jsx](src/components/liveStage.jsx) |
