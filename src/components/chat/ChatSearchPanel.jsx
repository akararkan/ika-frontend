...
export function ChatSearchPanel({ convo, onClose, onJump }) {
  const { getConvo } = useChat()
  const canScopeLocal = !!convo
  const [scope, setScope] = React.useState(canScopeLocal ? 'local' : 'global')
  ...
  React.useEffect(() => {
    ...
    setState('loading')
    const t = setTimeout(() => {
      const run = scope === 'local' && convo
        ? api.chat.messages.search(convo.id, needle, 30, { signal: ctl.signal })
        ...
        {state === 'ready' && rows.map(m => {
          ...
          return (
            <button key={m.id} className="ch-hitrow"
              ...
              <Avatar initials={m.sender?.initials} color={m.sender?.avc} size={32} src={m.sender?.profileImage}/>
              <span className="ch-hitrow-main">
                <span className="ch-hitrow-top">
                  <b dir="auto">{m.sender?.full || 'Member'}</b>
                  {other && <span className="ch-hitrow-in" dir="auto">in {other.displayTitle}</span>}
