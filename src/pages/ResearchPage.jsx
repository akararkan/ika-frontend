      ...
      <div className="col-main">
        <div className="phead">
          <div>
            ...
            <h1>Scholarly <em>Research</em> <span className="phead-ar" lang="ar" dir="rtl">البحوث</span></h1>
        ...
        {loading ? <Loader label="Loading research…"/>
          : !list.length ? (
            ...
            <>
              {!!list.length && (
                <article className="r-hero rise" onClick={() => navigate(`/research/${list[0].id}`)}>
                  <span className="rh-ph" style={{ background: list[0].cover }}/>
                  <div className="rh-panel">
                    <span className="rh-meta font-mono">{list[0].irc || ''}{list[0].irc && list[0].status ? ' · ' : ''}<b className={'rh-st is-' + String(list[0].status || '').toLowerCase()}>{String(list[0].status || '').toLowerCase()}</b></span>
                    <h3 dir="auto">{list[0].title}</h3>
                    {list[0].abstract && <p className="r-abs">{list[0].abstract}</p>}
                  </div>
                </article>
              )}
              <div className="r-list" data-view={view}>{list.slice(1).map((r, i) => RCard(r, i))}</div>
            </>
          )}
      </div>
