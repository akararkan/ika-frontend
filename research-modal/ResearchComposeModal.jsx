...
const RM_SECTIONS = [
  { id:'essentials',   icon:'doc',       label:'Essentials',      hint:'Abstract · description' },
...
/* One content section. Module-level (stable identity) so the rich-text editors
   ...
   can scroll-spy / jump to it; `active` mirrors the rail's rubric tick on the card. */
function RmSection({ id, icon, title, tag, active, innerRef, children }) {
  return (
    <section className={'rm-sec' + (active ? ' is-live' : '')} data-sec={id} ref={innerRef}>
  ...
  return (
    <div className="overlay open rm-overlay" onClick={e => { if (e.target === e.currentTarget && !busy) onClose() }}>
      <div className="modal rm-modal" role="dialog" aria-label={isEdit ? 'Edit research' : 'Publish research'}>
        {/* Header — kicker + title + progress bar (the cover lives in the hero below) */}
        <header className="rm-hd">
          <div className="rm-hd-text">
            <span className="rm-kicker">{isEdit ? 'Edit manuscript' : 'New manuscript'}</span>
        ...
        <div className="rm-body">
          ...
          <nav className="rm-nav" aria-label="Sections">
            <div className="rm-nav-cap">Contents <b lang="ar" dir="rtl">الفهرس</b></div>
            <ul className="rm-nav-list">
              {RM_SECTIONS.map((s, i) => (
                <li key={s.id}>
                  <button type="button" className={'rm-nav-item' + (activeSec === s.id ? ' on' : '')} onClick={() => goto(s.id)}>
                    <span className="rm-no font-mono">{String(i + 1).padStart(2, '0')}</span>
          ...
          {/* Full-bleed cover hero — set the cover image + title right here; this is
              exactly how the published masthead opens (image-first, title over the photo). */}
          {!busy && (
            <div className={'rm-hero' + (coverShown ? '' : ' empty')}
              style={coverShown ? { background:`center/cover no-repeat url("${coverShown}")` } : undefined}>
              {!coverShown && <span className="rm-hero-pattern" aria-hidden="true"/>}
              <span className="rm-hero-scrim" aria-hidden="true"/>
              <div className="rm-hero-bar">
                <span className="rm-hero-eyebrow">{isEdit ? 'Edit manuscript' : 'New manuscript'}</span>
                <div className="rm-hero-acts">
                  <button type="button" className="rm-hero-btn" onClick={() => coverRef.current?.click()}>
                    <Icon name="image" className="xs"/>{coverShown ? 'Replace cover' : 'Add cover image'}
                  </button>
                  {coverShown && <button type="button" className="rm-hero-btn ghost" onClick={removeCover}><Icon name="close" className="xs"/>Remove</button>}
                </div>
              </div>
              <div className="rm-hero-content">
                <input className="rm-hero-title" dir="auto" placeholder="Your research title…"
                  value={title} onChange={e => setTitle(e.target.value)}/>
                <div className="rm-hero-by">
                  <Avatar initials={meInitials} color={me.avc} size={34} src={me.profileImage}/>
                  <span>{isEdit ? 'Editing as' : 'Publishing as'} <b>{me.full}</b></span>
                </div>
              </div>
              {coverFile && !canCover && <span className="rm-hero-warn"><Icon name="lock" className="xs"/> Cover image needs a Scholar / Researcher role.</span>}
            </div>
          )}
          ...
          {!busy && (
            <div className="rm-inner">
              <RmSection id="essentials" active={activeSec === 'essentials'} icon="doc" title="Essentials" tag="required" innerRef={el => (secRefs.current.essentials = el)}>
                <p className="rm-micro" style={{ marginTop:-4 }}>Your title is set on the cover hero at the top.</p>
              ...
              <RmSection id="discovery" active={activeSec === 'discovery'} icon="hash" title="Discovery" tag="how readers find it" innerRef={el => (secRefs.current.discovery = el)}>
                <div className="rm-field">
              ...
              <RmSection id="media" active={activeSec === 'media'} icon="image" title="Media" tag="shown on research cards" innerRef={el => (secRefs.current.media = el)}>
                  ...
                  <div className="rm-field">
                    ...
                    {coverShown ? (
                      <div className="rm-media-prev">
                        <figure className="rm-cover-plate">
                          <div className="rm-cover" style={{ background:`center/cover no-repeat url("${coverShown}")` }}/>
                          <figcaption><b>Cover plate</b><i>framed on the research card &amp; reader</i></figcaption>
                        </figure>
                      ...
                      <button type="button" className="rm-drop tall" onClick={() => coverRef.current?.click()}>
                        ...
                        <span className="rm-drop-t">Add a cover plate</span>
                        <span className="rm-drop-h">PNG / JPG · landscape · framed on the card</span>
                      </button>
                    )}
              ...
              <RmSection id="contributors" active={activeSec === 'contributors'} icon="users" title="Contributors" tag="co-authors · advisors · translators" innerRef={el => (secRefs.current.contributors = el)}>
                <ContributorsField value={contribs} onChange={setContribs} meId={me.id}/>
              </RmSection>

              <RmSection id="sources" active={activeSec === 'sources'} icon="cite" title="Sources & references" innerRef={el => (secRefs.current.sources = el)}>
                {sources.map((s, i) => (
              ...
              <RmSection id="files" active={activeSec === 'files'} icon="paperclip" title="Files & figures" tag="paper PDF · datasets · figures" innerRef={el => (secRefs.current.files = el)}>
                {existingMedia.map(m => (
              ...
              <RmSection id="publishing" active={activeSec === 'publishing'} icon="settings" title="Publishing" tag="permissions & schedule" innerRef={el => (secRefs.current.publishing = el)}>
        ...
        <footer className="rm-ftr">
          ...
          <div className="rm-ftr-actions">
            ...
            <button type="button" className={'btn btn-primary' + (!isEdit && publishNow ? ' rm-go' : '')} disabled={busy || !title.trim()} onClick={submit}><Icon name="feather" className="sm"/>{ctaLabel}</button>
          </div>
        </footer>
      </div>
    </div>
